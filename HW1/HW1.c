...
#define TEST_FILENAME "testing_plot.png"
    ...
    */
    if(do_single_step) {
        ...
        while(1) {
            ...
            printf("output values: %lf, %lf\n", output[0], output[1]);
            printf("(%.02lf, %.02lf) is in Region ", input[1], input[2]);
            // print region number based on binary output
            if(output[0] < 0.0000001) {
                if(output[1] < 0.0000001) printf("1\n");
                else printf("2\n");
            } else {
                if(output[1] < 0.0000001) printf("3\n");
                else printf("4\n");
            }
            printf("--------------------------------------------------\n");
        }
...
void Train(double** w, int iterations, double lambda, int numInputs, int numOutputs, double minCoord, double maxCoord)
{
    ...
    for(int i = 0; i < iterations; i++) {
        ...
        // update weights
        if((correct[0] - output[0] < 0.000001) && (correct[1] - output[1] < 0.000001)) {
            inarow++;
            numcorrect++;
        } else {
            inarow = 0;
...
SEE ATTACHED: regions.png
*/
void CorrectOutput(double* t, double x, double y)
{
    if (y > fabs(x)) { // GREEN REGION
        t[0] = 0;
        t[1] = 0;
    } else
    if (x >= fabs(y)) { // PURPLE REGION (line-inclusive)
        t[0] = 0;
        t[1] = 1;
    } else
    if (y < x && x < -y) { // BLACK REGION
        t[0] = 1;
        t[1] = 0;
    } else if(x <= y && y <= -x) { // RED REGION (line-inclusive)
        t[0] = 1;
        t[1] = 1;
    } else {
        // funny printf
        printf("Congratulations, it broke. This shouldn't be possible.\n");
        exit(EXIT_FAILURE);
    }
}
